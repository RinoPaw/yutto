...
def add_download_arguments(parser: argparse.ArgumentParser) -> None:
    ...
    config.add_argument("--config", type=path_from_cli, help="配置文件路径")
...
def _add_basic_arguments(parser: argparse.ArgumentParser) -> None:
    ...
    group.add_argument("-j", "--jobs", type=int, help="同时下载的视频数量，默认为 1")
    group.add_argument("--fetch-workers", type=int, help="批量解析时同时请求的最大 Worker 数")
    group.add_argument(
        "-q",
        "--video-quality",
    ...
    group.add_argument("-w", "--overwrite", action="store_true", help="强制覆盖已下载内容")
    group.add_argument(
        "-x",
        "--proxy",
        help="设置代理（auto 为系统代理、no 为不使用代理、当然也可以设置代理值）",
    )
    group.add_argument("-d", "--dir", type=path_from_cli, help="下载目录，默认为运行目录")
    group.add_argument(
        "--tmp-dir",
        type=path_from_cli,
    ...
    group.add_argument("-tp", "--subpath-template", help="多级目录的存储路径模板")
    group.add_argument(
        "-af",
        "--alias-file",
        dest="aliases",
    ...
    group.add_argument("--download-interval", type=int, help="设置下载间隔，单位为秒")
    group.add_argument(
        "--banned-mirrors-pattern",
        help="禁用下载链接的镜像源，使用正则匹配",
    )
    group.add_argument("--vip-strict", action="store_true", help="启用严格检查大会员生效")
    group.add_argument("--login-strict", action="store_true", help="启用严格检查登录状态")
    group.add_argument("--no-color", action="store_true", help="不使用颜色")
    group.add_argument("--no-progress", action="store_true", help="不显示进度条")
    group.add_argument("--debug", action="store_true", help="启用 debug 模式")
...
def _add_auth_arguments(parser: argparse.ArgumentParser) -> None:
    ...
    group.add_argument("--auth-file", type=path_from_cli, help="认证信息文件路径")
    group.add_argument("--auth-profile", help="认证信息 profile 名称，默认 default")
...
def _add_danmaku_arguments(parser: argparse.ArgumentParser) -> None:
    group = parser.add_argument_group("danmaku", "弹幕设置参数")
    group.add_argument("--danmaku-font-size", type=int, help="弹幕字体大小")
    group.add_argument("--danmaku-font", help="弹幕字体")
    group.add_argument("--danmaku-opacity", type=float, help="弹幕不透明度")
    group.add_argument(
        "--danmaku-display-region-ratio",
        type=float,
        help="弹幕显示区域与视频高度的比例",
    )
    group.add_argument("--danmaku-speed", type=float, help="弹幕速度")
    group.add_argument("--danmaku-block-top", action="store_true", help="屏蔽顶部弹幕")
    group.add_argument("--danmaku-block-bottom", action="store_true", help="屏蔽底部弹幕")
    group.add_argument("--danmaku-block-scroll", action="store_true", help="屏蔽滚动弹幕")
    group.add_argument("--danmaku-block-reverse", action="store_true", help="屏蔽逆向弹幕")
    group.add_argument(
        "--danmaku-block-fixed",
        action="store_true",
        help="屏蔽固定弹幕（顶部、底部）",
    )
    group.add_argument("--danmaku-block-special", action="store_true", help="屏蔽高级弹幕")
    group.add_argument("--danmaku-block-colorful", action="store_true", help="屏蔽彩色弹幕")
    group.add_argument(
...
def _add_batch_arguments(parser: argparse.ArgumentParser) -> None:
    ...
    group.add_argument("--skip-preview", action="store_true", help="跳过预告片")
    group.add_argument(
