...
class UgcVideoBatchExtractor(BatchExtractor):
    ...
    REGEX_AV = re.compile(r"https?://www\.bilibili\.com/video/av(?P<aid>[0-9]+)/?")
    REGEX_BV = re.compile(r"https?://www\.bilibili\.com/video/(?P<bvid>(bv|BV)\w+)/?")

    REGEX_AV_ID = re.compile(r"av(?P<aid>[0-9]+)(\?p=(?P<page>[0-9]+))?")
    REGEX_BV_ID = re.compile(r"(?P<bvid>(bv|BV)\w+)(\?p=(?P<page>[0-9]+))?")
