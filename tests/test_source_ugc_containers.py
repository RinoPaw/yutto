...
from yutto.types import CollectionId, FId, MId, SeriesId
...
def test_favourite_preserves_folder_owner_and_item_titles(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    media = asyncio.run(UgcFavSource(id=FId("456")).resolve(None, options))  # type: ignore[arg-type]

    assert media.metadata.owner == "收藏者"
    assert media.metadata.mid == MId("999")
    assert [video.metadata.title for video in media.items] == ["收藏里的单P标题", "收藏里的多P标题"]
    assert media.items[0].metadata.show_title == "原始单P标题"
    assert media.items[0].items[0].metadata.title == "收藏里的单P标题"
    assert media.items[0].items[0].metadata.show_title == "收藏里的单P标题"
    assert [page.metadata.title for page in media.items[1].items] == ["P1", "P2"]
    assert all(page.metadata.show_title == "原始多P标题" for page in media.items[1].items)
